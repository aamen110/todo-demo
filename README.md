...
```
git clone https://github.com/aamen110/todo-demo.git
cd todo-demo
...
```
docker run --name mysql -e MYSQL_ROOT_PASSWORD=password -e MYSQL_DATABASE=tododb -e MYSQL_USER=user -e MYSQL_PASSWORD=pass -d mysql:5.7
```
...
```
docker run -p 8080:8080 --name todo --link mysql -d kaka/todo-demo:1.0.0
```
